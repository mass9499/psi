            ...
            if($this->rest->get_request_method() == "POST" && $this->rest->_request){            
                ...
                foreach ($level as $key => $value) {
                    $level_list[] = array(
                        ...
                        'image_name' => base_url()."files/level/".$value->image_name
                    );
                }
                if($parent)
...
// print_r($data); die;
            ...
            $level_id      =  $this->input->post('level_id');
            $ear_side      =  $this->input->post('ear_side');
            ...
            $test_id = $this->Api_Model->insert_test_history($insert_answer);

        }

            return $this->rest->response(200,array('test_id' => $test_id),'success',0);
    ...
    public function end_test()
    {
        ...
        $this->Api_Model->update_test_history($test_id,$update_answer);
    ...
    { 
        if($this->rest->get_request_method() == "POST" && $this->rest->_request){  
            ...
            $test_history = $this->Api_Model->get_test_id($test_id);
            // pr($test_history);

            // die;
            // $questions = $this->Api_Model->get_questions_id($test_history->level_id);
           
            // $answers = $this->Api_Model->get_answers_id($test_id);
           
            $total_crt_ans = $this->Api_Model->get_passcount_id($test_id);
            //$total_crt_ans = count($pass_count);

            $total_answers = $this->Api_Model->total_answers($test_id);
            //$total_answers = count($total_answer);

            // $percentage = round(AVG($total_crt_ans)*100);
            $ans_percentage = ($total_crt_ans / $total_answers) * 100;

                $update_answer = array(
                    'total_questions'       => $total_answers,
                    'total_answers'         => $total_crt_ans,
                    'percentage'            => round($ans_percentage),
                    'end_time'              => date("Y-m-d H:i:s")
                );
    ...
    public function patient_add(){
            ...
            if($this->rest->get_request_method() == "POST" && $this->rest->_request){        
                ...
                $data =  array(
                                   ...
                                   "left_normal_hear"       => $this->input->post('left_normal_hear'),
                                   "left_hearing_device"    => $this->input->post('left_hearing_device'),
                                   "left_implant_date"      => $this->input->post('left_implant_date'),
                                   "right_normal_hear"      => $this->input->post('right_normal_hear'),
                                   "right_hearing_device"   => $this->input->post('right_hearing_device'),
                                   "right_implant_date"     => $this->input->post('right_implant_date'),
                ...
                //                     'l_hybrid_system'      =>    @$this->input->post('l_hybrid_system'),
                //                     'r_hybrid_system'      =>    @$this->input->post('r_hybrid_system'),
                //                     'l_adhesive_bone'      =>    @$this->input->post('l_adhesive_bone'),
                //                     'r_adhesive_bone'      =>    @$this->input->post('r_adhesive_bone')
                //             );
                            
                //     //
                //   if($datas){ $this->Api_Model->save('ear_implant',$datas);}  
                
                 return $this->rest->response(200,array("patient_id" => $inserted),'success',0);
                 }   
              }
              return $this->rest->response(400,array(),'bad request',1); 
        }
    ...
    public function patient_edit(){
            ...
            if($this->rest->get_request_method() == "POST" && $this->rest->_request){  
                ...
                $data["left_normal_hear"]       = @$this->input->post('left_normal_hear');
                $data["left_hearing_device"]    = @$this->input->post('left_hearing_device');
                $data["left_implant_date"]      = @$this->input->post('left_implant_date');
                $data["right_normal_hear"]      = @$this->input->post('right_normal_hear');
                $data["right_hearing_device"]   = @$this->input->post('right_hearing_device');
                $data["right_implant_date"]     = @$this->input->post('right_implant_date');
                ...
                //  if($this->input->post('l_hybrid_system'))     $patient_ear_data['l_hybrid_system']    = $this->input->post('l_hybrid_system');
                //  if($this->input->post('r_hybrid_system'))     $patient_ear_data['r_hybrid_system']    = $this->input->post('r_hybrid_system');
                 
                 
                //  if($this->input->post('l_adhesive_bone'))     $patient_ear_data['l_adhesive_bone']    = $this->input->post('l_adhesive_bone');
                //  if($this->input->post('r_adhesive_bone'))     $patient_ear_data['r_adhesive_bone']    = $this->input->post('r_adhesive_bone'); 
    ...
    public function test_results(){
         if($this->rest->get_request_method() == "POST" && $this->rest->_request){  
                ...
                $doctor_id = $this->input->post('doctor_id');
                $result = $this->Api_Model->get_patient_results($doctor_id);
                $results =  array("result" => $result);
                    return $this->rest->response(200,$results,'success',0);
                }
                 else {
    ...
    public function test_results_view(){

            if($this->rest->get_request_method() == "POST" && $this->rest->_request){  

            $test_id =  $this->input->post('test_id');

            $result['test_detail'] = $this->Api_Model->get_test_details($test_id);
            $result['questions'] = $this->Api_Model->get_test_questions($test_id);
