...
class question extends MX_Controller
{
    ...
    public function edit($id) {
        ...
        if($this->input->server("REQUEST_METHOD")=="POST"){        
            ...
            $question_data['name']=$this->input->post('name');
            $question_data['timing']=$this->input->post('timing');
            $audio_name=$this->input->post('old_audio');
           ...
           if($this->input->post('old_answer')){
                $total_count = count($this->input->post('old_answer'));
               
                for ($i=0; $i < $total_count; $i++) { 
                  //  pr($this->input->post('correct')); die;
                    $correct_answer = @$this->input->post('correct')[$i] ?  1 : 0 ;
